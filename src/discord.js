...
const webhookUrl =
	"https://discordapp.com/api/webhooks/708351218388435015/PWLB31ajkbpSuGMV9HF55VBdq0v7SRMzOKEuOC8wvfz8Ya_lsuDI2lTmoL1DcbYW3f2C";
...
client.on("message", (receivedMessage) => {
	...
	let filter = (msg) => {
		return (
			msg.content.toLowerCase() == receivedMessage.content.toLowerCase()
		);
	};

	if (
		receivedMessage.author == client.user ||
		(channelSource != shoutbox_channel && webHookId != webhookIdrc) ||
		(receivedMessage.author.bot && webHookId != webhookIdrc)
	) {
		return;
	}
	...
	if (receivedMessage.reference != null) {
		receivedMessage.channel.messages
			.fetch(receivedMessage.reference.messageID)
			.then(function (data) {
...
client.on("ready", () => {
	...
	discordToTchat.botLogin().then(function (data) {
		sendEmbed(
			"Le Bot est **en ligne** sur le tchat de TIplanet!",
			botLogChannel
		);
	...
	if (primaryCommand == "tibot") {
		if (arguments[0] == "login") {
			discordToTchat
				...
				.then(function (data) {
					sendEmbed(
						"Le Bot est désormais **en ligne** sur le tchat de TIplanet!",
						botLogChannel
					);
				})
				.catch(function (error) {
			...
			discordToTchat
				...
				.then(function (data) {
					sendEmbed(
						"Le Bot est désormais **HORS LIGNE** sur le tchat de TIplanet!",
						botLogChannel
					);
				})
				.catch(function (error) {
			...
			if (
				receivedMessage.member.roles.cache.has(roleAdmin) ||
				receivedMessage.member.roles.cache.has(roleModoG)
			) {
				const embed = new Discord.MessageEmbed()
